...
# def custom_collate(batch):
#     """
#     각 item: (x, y, x_mark, y_mark, last_emb, full_prompt, anchor_avg)
#       x: [L, N], y: [...], x_mark: [...], last_emb: [d_llm, N], full_prompt: [T_i, d_llm, N], anchor_avg: [N, d_llm]
#     """
#     xs, ys, xms, yms, lasts, fps, anchors = zip(*batch)

#     x       = torch.as_tensor(np.array(xs))
#     y       = torch.as_tensor(np.array(ys))
#     x_mark  = torch.as_tensor(np.array(xms))
#     y_mark  = torch.as_tensor(np.array(yms))
#     last    = torch.as_tensor(np.array(lasts))          # 기대: [B, d_llm, N] or 브로드캐스트 가능
#     full_p  = pad_prompt_embeddings(fps)                # [B, T_max, d_llm, N]
#     anchor  = torch.as_tensor(np.array(anchors))        # [B, N, d_llm] (저장 논리에 맞춰)

#     return x, y, x_mark, y_mark, last, full_p, anchor
...
def main():
    ...
    engine.model.visualize_embeddings(
        enc_pre, pr_pre, cr_pre, full_prompt_pre,
        perplexity=30.0, save_path="embeddings_viz_pre.png"
    )
    print("==> visualized pre-training embeddings (first batch)")

    # 1) prompt 전체 토큰 평균 → [N, d_llm]
    fp_pre = full_prompt_pre[0]                       # [T_max, d_llm, N]
    fp_avg_pre = fp_pre.mean(dim=0).permute(1, 0)     # [N, d_llm]

    # 2) cross, prompt_last 벡터 정리
    cross_vec_pre = cr_pre.squeeze(0)                 # [N, C]
    pr_vec_pre = pr_pre.permute(0, 2, 1).squeeze(0)   # [N, d_llm]

    # 3) 차원 매칭 (to common_dim)
    proj_fp = (nn.Linear(fp_avg_pre.shape[1], cross_vec_pre.shape[1]).to(device)
               if fp_avg_pre.shape[1] != cross_vec_pre.shape[1] else nn.Identity())
    proj_pr = (nn.Linear(pr_vec_pre.shape[1],  cross_vec_pre.shape[1]).to(device)
               if pr_vec_pre.shape[1]  != cross_vec_pre.shape[1]  else nn.Identity())
    fp_avg_proj_pre = (proj_fp(fp_avg_pre.to(device)) if not isinstance(proj_fp, nn.Identity) else fp_avg_pre.to(device))
    pr_vec_proj_pre = (proj_pr(pr_vec_pre.to(device)) if not isinstance(proj_pr, nn.Identity) else pr_vec_pre.to(device))

    # 4) Pairwise distance MSE (pre)
    pd_mse_pre_fp = compute_pairwise_distance_mse(fp_avg_proj_pre.detach().cpu(), cross_vec_pre.detach().cpu())
    pd_mse_pre_pr = compute_pairwise_distance_mse(pr_vec_proj_pre.detach().cpu(), cross_vec_pre.detach().cpu())
    print(f"Pre-training Pairwise Distance MSE(fp): {pd_mse_pre_fp:.6f}")
    print(f"Pre-training Pairwise Distance MSE(pr): {pd_mse_pre_pr:.6f}")
    np.save("pairwise_mse_pre_fp.npy", np.array([pd_mse_pre_fp]))
    np.save("pairwise_mse_pre_pr.npy", np.array([pd_mse_pre_pr]))
    ...
    plot_pairwise_distance_heatmaps(
        ...
        pr_vec_proj_pre,
        cross_vec_pre,
        save_path="heatmap_prompt_cross_pre.png"
    )

    # 6) Cosine similarity (pre)
    sim_pre = compute_cosine_similarity(pr_pre, cr_pre)
    np.save("sim_pre.npy", sim_pre.detach().cpu().numpy())
    print(f"Pre-training avg cosine similarity (prompt_last vs cross_out): {sim_pre.mean().item():.4f}")
   ...
   # ===== Post-training: 첫 배치 임베딩/지표/시각화 (예전 로직 유지) =====
    ...
    engine.model.visualize_embeddings(
        enc_post, pr_post, cr_post, full_prompt_post,
        perplexity=30.0, save_path="embeddings_viz_post.png"
    )
    print("==> visualized post-training embeddings (first batch)")

    # --- Post-training 지표 (이전 로직 동일) ---
    # 1) prompt 전체 토큰 평균 → [N, d_llm]
    ...
    # 3) 차원 매칭 (common_dim)
    proj_fp_post = (nn.Linear(fp_avg_post.shape[1], cross_vec_post.shape[1]).to(device)
                if fp_avg_post.shape[1] != cross_vec_post.shape[1] else nn.Identity())
    proj_pr_post = (nn.Linear(pr_vec_post.shape[1],  cross_vec_post.shape[1]).to(device)
                if pr_vec_post.shape[1]  != cross_vec_post.shape[1]  else nn.Identity())

    fp_avg_proj_post = (proj_fp_post(fp_avg_post.to(device)) if not isinstance(proj_fp_post, nn.Identity) else fp_avg_post.to(device))
    pr_vec_proj_post = (proj_pr_post(pr_vec_post.to(device)) if not isinstance(proj_pr_post, nn.Identity) else pr_vec_post.to(device))

    # 4) Pairwise distance MSE (post)
    pd_mse_post_fp = compute_pairwise_distance_mse(fp_avg_proj_post.detach().cpu(), cross_vec_post.detach().cpu())
    pd_mse_post_pr = compute_pairwise_distance_mse(pr_vec_proj_post.detach().cpu(), cross_vec_post.detach().cpu())
    print(f"Post-training Pairwise Distance MSE(fp): {pd_mse_post_fp:.6f}")
    print(f"Post-training Pairwise Distance MSE(pr): {pd_mse_post_pr:.6f}")
    np.save("pairwise_mse_post_fp.npy", np.array([pd_mse_post_fp]))
    np.save("pairwise_mse_post_pr.npy", np.array([pd_mse_post_pr]))
    ...
    plot_pairwise_distance_heatmaps(
        ...
        pr_vec_proj_post,
        cross_vec_post,
        save_path="heatmap_prompt_cross_post.png"
    )
    # 6) Cosine similarity (post)
    sim_post = compute_cosine_similarity(pr_post, cr_post)
    np.save("sim_post.npy", sim_post.detach().cpu().numpy())
    print(f"Post-training avg cosine similarity (prompt_last vs cross_out): {sim_post.mean().item():.4f}")

    # 7) 분포 시각화 (pre vs post)
    plot_similarity_distributions(sim_pre, sim_post, save_path="sim_distribution.png")
